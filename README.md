...
The corporate core blocks user account changes, user-controlled Activation Lock, personal iCloud backup, document, application, Keychain, Photos and Private Relay services, AirDrop, USB Files access, computer pairing and wallpaper changes. AirDrop is also treated as an unmanaged destination as defence in depth. It deliberately leaves screenshots and joining non-managed Wi-Fi networks available. Recovery-mode erase/restore remains the technician recovery path when normal host pairing is unavailable.
