...
The corporate core blocks user account changes, user-controlled Activation Lock, personal iCloud backup, document, application, Keychain, Photos and Private Relay services, and AirDrop. AirDrop is also treated as an unmanaged destination as defence in depth. It deliberately does not block screenshots or all host pairing. The remaining omitted controls have material usability, recovery, contact-sync or technician-support consequences and must be decided with the customer after a real pilot. The profile also explicitly leaves joining non-managed Wi-Fi networks available.
