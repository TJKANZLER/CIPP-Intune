...
The corporate core blocks user account changes, user-controlled Activation Lock, and personal iCloud backup, document, application, Keychain, Photos and Private Relay services. It deliberately does not block screenshots or AirDrop or block all host pairing. AirDrop is treated as an unmanaged destination and managed documents cannot open in unmanaged apps. The remaining omitted controls have material usability, recovery, contact-sync or technician-support consequences and must be decided with the customer after a real pilot. The profile also explicitly leaves joining non-managed Wi-Fi networks available.
